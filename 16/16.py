...
class MainWindow:
    def __init__(self, title, size):
        ...
        file_menu.add_command(label="Лидеры", command=self.ctx_leader)
        main_menu = Menu()
    ...
    def StartWidget(self):
        ...
        self.show_btn = MyButton("☼", "Arial 12", 310, 198, 35, self, "show")
        self.loose_password = MyButton("Забыли пароль?", "Arial 8", 150, 300, 100, self, "password")
...
стрелками на клавиатуре", "Arial 15", self.main_color, 10, 650)
    ...
    def ctx_leader(self):
        if not self.visible:
            self.show_table_leader()
            self.visible = True
        else:
            self.delete_table_leader()
            self.visible = False
    ...
    def about(self):
        path = "C:/dev/projects/ModuleThree/about.txt"
        subprocess.run(['start', 'notepad', path], shell=True)
...
class MyButton:
    ...
    def back(self):
        ...
        self.window.start_window = MainWindow("ВХОД", "400x400")
        self.window.start_window.StartWidget()
        self.window.start_window.VisibleWindow()

    def new_user(self):
        self.user_login = simpledialog.askstring("Добавление пользователя", "Введите логин нового пользователя:")
        self.user_password = simpledialog.askstring("Добавление пользователя", "Введите пароль для нового пользователя:")
        self.db = ConnectDB("C:/dev/projects/ModuleThree/db.db")
        self.sql = self.db.SelectSQL(f"INSERT INTO users (login, password) VALUES ('{self.user_login}', '{self.user_password}');")
        self.db.CloseDB()
        messagebox.showinfo("Добавление пользователя", f"Пользователь {self.user_login} добавлен!")

    def del_user(self):
        self.user_login = simpledialog.askstring("Удаление пользователя", "Введите логин пользователя:")
        result = messagebox.askyesno("Подтверждение удаления", "Вы уверены?")
        if result:
            self.db = ConnectDB("C:/dev/projects/ModuleThree/db.db")
            self.sql = self.db.SelectSQL(f"DELETE from users WHERE login='{self.user_login}'")
            self.db.CloseDB()
            messagebox.showinfo("Удаление пользователя", f"Пользователь {self.user_login} удален!")
    
    def update_user(self):
        self.user_login = simpledialog.askstring("Обновление пользователя", "Введите логин пользователя:")
        self.user_password = simpledialog.askstring("Обновление пользователя", "Введите новый пароль для пользователя:")
        result = messagebox.askyesno("Подтверждение изменения", "Вы уверены?")
        if result:
            self.db = ConnectDB("C:/dev/projects/ModuleThree/db.db")
            self.sql = self.db.SelectSQL(f"UPDATE users SET password = '{self.user_password}' WHERE login='{self.user_login}'")
            self.db.CloseDB()
            messagebox.showinfo("Изменение пользователя", f"Пользователю {self.user_login} задан новый пароль: {self.user_password}")

    def remember_password(self):
        good = messagebox.showerror("Ну ты молодец.", "Надо было запоминать.")
        if good:
            print("+")
            self.user_login = simpledialog.askstring("Обновление пользователя", "Введите логин пользователя:")
            self.user_password = simpledialog.askstring("Обновление пользователя", "Введите новый пароль для пользователя:")
